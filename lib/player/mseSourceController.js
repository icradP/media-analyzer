...
function filterH264SampleNalUnits(payload, { dropAud = true, dropFiller = true, requireVcl = true } = {}) {
...
function buildH264Fmp4FromAnalysis({ mediaInfo, result, pushLog }) {
  ...
  playableSamples[0].data = prependParameterSetsToIdrSample(playableSamples[0].data, avcC);
  playableSamples[0].nalTypes = h264PayloadNalTypes(playableSamples[0].data);
