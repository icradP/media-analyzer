...
} from "../browser/videoDecodeOrchestrator.js";
...
export class VideoSegmentPlayer {
    constructor({
        ...
        this.canvas = canvas || this.renderer?.canvas || null;
        ...
        this.decodeStrategy = decodeStrategy === "frame-step" ? "frame-step" : "gop";
        this.enableFrameStepFallback = enableFrameStepFallback !== false;
    ...
    async playSegment({ startSec, endSec, frames = null, codecString = null } = {}) {
        ...
        if (this.decodeStrategy === "frame-step") {
            if (!this.canvas) throw new Error("canvas is required for frame-step video decode.");
            this.clock.start(firstPts);
            this.status(`Video segment frame-step play: frames=${realFrames.length}, pts=[${start.toFixed(3)}, ${end.toFixed(3)}], rate=${this.clock.playbackRate}x`);
            try {
                const out = await this.playFrameStepSegment({
                    frames: realFrames,
                    startSec: start,
                    endSec: end,
                    token,
                ...
                this.status("Video segment frame-step play done.");
                this.state = "idle";
                return {
                    stopped: false,
    ...
    async playFrameStepSegment({ frames, startSec, endSec, token }) {
        const sorted = frames.slice().sort(compareFramesByTimeThenIndex);
        const fallbackFrames = collectVideoFrames(this.mediaInfo).map(wrapFrameForSingleFrameDecode);
        let rendered = 0;
        for (const frame of sorted) {
            if (token !== this.token) return { stopped: true, rendered };
            const idx = frameIndex(frame);
            if (idx === null) continue;
            const ptsTime = frameTimeSec(frame, startSec);
            const delayMs = this.clock.delayMsForMediaTime(ptsTime);
            if (delayMs > 1) await sleep(delayMs);
            if (token !== this.token) return { stopped: true, rendered };
            let out = null;
            try {
                out = await decodeVideoFrameWithStrategies({
                    mediaInfo: this.mediaInfo,
                    targetFrameIndex: idx,
                    stream: this.stream,
                    canvas: this.canvas,
                    returnVideoFrame: true,
                });
            } catch (err) {
                if (!this.enableFrameStepFallback) throw err;
                try {
                    out = await decodeGopByMseFallback({
                        targetFrame: wrapFrameForSingleFrameDecode(frame),
                        allFrames: fallbackFrames,
                        result: this.primary,
                        fileBytes: this.primary?.formatSpecific?.fileData || null,
                        canvas: this.canvas,
            ...
            rendered += 1;
            const decodedFrame = out?.frame || null;
            try {
                if (this.onFrame) this.onFrame({
                    ...
                    strategy: out?.strategy || out?.mode || "frame-step",
                    codec: out?.codec || null,
                    startSec,
                    endSec,
                });
            } finally {
                closeVideoFrame(decodedFrame);
            }
        }
    ...
    async decodeGroup({ group, codecString, token }) {
        ...
        for (const attempt of attempts) {
            ...
            try {
                ...
                if (attempt?.name && attempt.name !== "reference-chain") {
                    this.status(`Video segment decode strategy: ${attempt.name}`);
                }
                return;
            } catch (err) {
                lastErr = err;
                attemptErrors.push({
                    strategy: attempt?.name || "unknown",
    ...
    async decodeGroupAttempt({ group, plan, attempt, codecString, token }) {
        const visibleFrameIndexes = new Set(group.frames.map(frameIndex).filter((idx) => idx !== null));
        const timestampMeta = new Map();
        const encodedFrames = Array.isArray(attempt?.encodedFrames) ? attempt.encodedFrames : plan.encodedFrames;
        const metas = Array.isArray(attempt?.frameMeta)
            ? attempt.frameMeta
            : (Array.isArray(plan.encodedFrameMeta) ? plan.encodedFrameMeta : []);
        const description = attempt?.description ?? plan.description;
        const attemptName = attempt?.name || "reference-chain";
        const timestampStepUs = 33333;
        let lastQueuedChunk = null;
        let decodeError = null;
        let decodeErrorAt = null;
        let decodedVisibleCount = 0;
        const decodedVisibleFrameIndexes = [];
        ...
        const decoder = new VideoDecoder({
            output: (frame) => {
                ...
                decodedVisibleCount += 1;
                decodedVisibleFrameIndexes.push(idx);
        ...
        // Match the single-frame decode path: give WebCodecs one task turn to
        // deliver already-decoded frames before flush can surface a tail error.
        ...
        try {
            ...
            if (token !== this.token) return;
            const wrapped = wrapDecodeError(err, {
            ...
            addDecodeProgressDiagnostics(wrapped, {
                decodedVisibleCount,
                decodedVisibleFrameIndexes,
                visibleFrameIndexes,
            });
            if (decodedVisibleCount > 0) {
                this.status(`Video segment decode warning: ${wrapped.message}; rendered ${decodedVisibleCount}/${visibleFrameIndexes.size} frames in GOP.`);
                console.warn("[VideoSegmentPlayer] continuing after partial GOP flush failure", wrapped);
        ...
        if (decodeError && token === this.token) {
            const wrapped = wrapDecodeError(decodeError, {
            ...
            if (decodedVisibleCount > 0) {
                this.status(`Video segment decode warning: ${wrapped.message}; rendered ${decodedVisibleCount}/${visibleFrameIndexes.size} frames in GOP.`);
                console.warn("[VideoSegmentPlayer] continuing after partial GOP decoder error", wrapped);
                if (this.decoder === decoder) this.decoder = null;
                try {
                    decoder.close();
                } catch {
...
function wrapFrameForSingleFrameDecode(frame) {
    const raw = unwrapFrame(frame);
    const idx = frameIndex(raw);
    return {
        ...(raw || {}),
        index: idx,
        _mediaType: "video",
        _rawFrame: raw,
        _isKeyFrame: isKeyFrame(raw),
    };
}
...
function wrapDecodeError(err, { message, stage, codecString, plan, attemptName = null, encodedFrames = null, chunk }) {
    const wrapped = new Error(`${message}: ${err?.message || String(err)}`);
    wrapped.cause = err;
    wrapped.diagnostics = {
        stage,
        codecString,
        decodeAttempt: attemptName,
        ...
        decodePlan: {
            ...
            encodedFrames: Array.isArray(encodedFrames) ? encodedFrames.length : (plan?.encodedFrames?.length ?? 0),
            referenceEncodedFrames: plan?.encodedFrames?.length ?? 0,
...
function addDecodeProgressDiagnostics(err, { decodedVisibleCount, decodedVisibleFrameIndexes, visibleFrameIndexes }) {
    if (!err?.diagnostics) return err;
    err.diagnostics.decodedVisibleCount = decodedVisibleCount;
    err.diagnostics.decodedVisibleFrameIndexes = Array.isArray(decodedVisibleFrameIndexes)
        ? decodedVisibleFrameIndexes.slice()
        : [];
    err.diagnostics.expectedVisibleFrameIndexes = Array.from(visibleFrameIndexes || []);
    return err;
}
