...
function mimeTypeForSaveExtension(ext) {
...
function blobMimeTypeForSave(fileName, mimeType) {
    const ext = extensionFromFileName(fileName);
    const raw = String(mimeType || "").trim().toLowerCase();
    if (raw === "video/mp4" || raw === "application/octet-stream") return raw || mimeTypeForSaveExtension(ext);
    return mimeTypeForSaveExtension(ext);
}

function buildSaveFilePickerOptions(fileName) {
    const ext = extensionFromFileName(fileName);
    const optionsList = [];
    if (ext === "mp4" || ext === "mov" || ext === "m4s") {
        optionsList.push({
            suggestedName: fileName,
            types: [{
                description: "MP4 Video",
                accept: { "video/mp4": [`.${ext}`] },
            }],
        });
    }
    if (ext) {
    ...
    optionsList.push({ suggestedName: fileName });
    return optionsList;
}
...
export async function pickSaveFileLocation(fileName) {
    if (!canUseSaveFilePicker()) return null;
    const safeName = sanitizeDownloadFileName(fileName, "media-capture.bin");
    let lastError = null;
    for (const options of buildSaveFilePickerOptions(safeName)) {
        try {
...
export async function writeBytesToFileHandle(handle, bytes, mimeType = "application/octet-stream") {
    if (!handle?.createWritable) throw new Error("Invalid file handle.");
    if (!(bytes instanceof Uint8Array) || bytes.byteLength <= 0) {
        throw new Error("No bytes available to save");
    }
    ...
    return {
        fileName: handle.name || "saved-file",
...
export function downloadBytesAsFile(bytes, fileName, mimeType = "application/octet-stream") {
    ...
    return { fileName: safeName, size: bytes.byteLength, method: "download" };
}

/**
 * Save bytes via native Save dialog when available, otherwise anchor download.
 * Pass `options.fileHandle` to write to a location chosen earlier (keeps user activation).
 */
export async function saveBytesAsFile(bytes, fileName, mimeType = "application/octet-stream", options = {}) {
    if (!(bytes instanceof Uint8Array) || bytes.byteLength <= 0) {
        throw new Error("No bytes available to save");
    }
    const safeName = sanitizeDownloadFileName(fileName, buildCapturedMediaFileName({ bytes }));
    const resolvedMime = blobMimeTypeForSave(safeName, mimeType);
    const preferPicker = options.preferSavePicker !== false;
    if (options.fileHandle) {
        const out = await writeBytesToFileHandle(options.fileHandle, bytes, resolvedMime);
        out.fileName = out.fileName || safeName;
        return out;
    }
    if (preferPicker && canUseSaveFilePicker()) {
        try {
            const handle = await pickSaveFileLocation(safeName);
            if (handle) return writeBytesToFileHandle(handle, bytes, resolvedMime);
        } catch (err) {
            if (err?.name === "AbortError") throw err;
            console.warn("showSaveFilePicker failed, falling back to download.", err);
        }
    }
    return downloadBytesAsFile(bytes, safeName, resolvedMime);
}
