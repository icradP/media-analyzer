...
import { flvVideoTagBodyBitLength } from "./flvMediaLabels.js";
...
export function buildFlvMetadataSummary(
    ...
    if (videoSeqHdr || scriptMeta || videoPayloadBytes > 0) {
        ...
        if (videoSeqHdr?.codec) {
            ...
            const p = scriptMeta.videocodecid;
            if (p === 7) out.video.codec = "H.264";
            else if (p === 12) out.video.codec = "H.265";
            else out.video.codec = `Unknown (${p})`;
    ...
    if (audioAsc || scriptMeta || audioPayloadBytes > 0) {
        ...
        if (audioAsc?._profile != null) {
            ...
            const p = scriptMeta.audiocodecid;
            if (p === 10) out.audio.codec = "AAC";
            else if (p === 2) out.audio.codec = "MP3";
            else out.audio.codec = `Unknown (${p})`;
