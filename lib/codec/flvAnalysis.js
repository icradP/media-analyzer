...
function inferFlvVideoCodecFromTags(tags) {
    for (const tag of Array.isArray(tags) ? tags : []) {
        if (tag?.tagType !== 9) continue;
        if (tag.codecId && tag.codecId !== "Unknown") return tag.codecId;
        if (Number.isFinite(Number(tag._codecId_value))) {
            return flvVideoCodecIdName(Number(tag._codecId_value));
        }
    }
    return undefined;
}
...
export function buildFlvAnalysisResult(
    ...
    if (hasVideo) {
        ...
        const codecFromTags = inferFlvVideoCodecFromTags(tags);
        sb.setFieldWithPriority("codecName", [
            sourceEntry(naluSpsInfo?.codecName ?? naluSpsInfo?.codecId, "sps"),
            sourceEntry(codecFromTags, "flv-tag"),
