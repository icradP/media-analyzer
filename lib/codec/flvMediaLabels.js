...
export function flvMetadataVideoCodecName(codecId) {
    const numeric = numericCodecId(codecId);
    if (numeric != null) return flvVideoCodecIdName(numeric);
    if (typeof codecId === "string" && codecId) return mp4SampleEntryTypeLabel(codecId);
    return undefined;
}

export function flvMetadataAudioCodecName(soundFormat) {
    const numeric = numericCodecId(soundFormat);
    if (numeric != null) return flvSoundFormatName(numeric);
    if (typeof soundFormat === "string" && soundFormat) {
        if (soundFormat === "mp4a") return "AAC";
        return soundFormat;
    }
    return undefined;
}
