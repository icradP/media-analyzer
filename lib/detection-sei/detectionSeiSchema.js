 ...
 * @property {boolean} [writeEmptyDetections]
 */
...
export function normalizeDetectionSeiProcessOptions(input = {}) {
    return {
        confidenceThreshold: clampThreshold(input.confidenceThreshold, 0.35),
        iouThreshold: clampThreshold(input.iouThreshold, 0.45),
        frameStride: Math.max(1, Math.min(120, Math.round(Number(input.frameStride) || 1))),
        replaceExisting: input.replaceExisting !== false,
        fallbackRunDetection: input.fallbackRunDetection === true,
        writeEmptyDetections: input.writeEmptyDetections === true,
        maxDetectionsPerFrame: Math.max(1, Math.min(100, Math.round(Number(input.maxDetectionsPerFrame) || 20))),
    };
}
