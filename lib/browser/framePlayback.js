...
function sliceG711SamplePayload(frame, fileData) {
    if (!frame) return null;
    const fs = frame?.formatSpecific || {};
    const fieldOffsets = fs?.fieldOffsets || {};
    // Prefer the audio-payload-only range produced by FLV (skips the 1-byte
    // soundFormat/soundRate/soundSize/soundType header). Falls back to `.data`
    // which the FLV audio parser also rewrites to point past that 1-byte header.
    const range = fieldOffsets.audioData || fieldOffsets.data;
    if (
...
function buildG711AudioBufferFromFrames(ctx, frames, mediaInfo) {
    ...
    for (const item of inputFrames) {
        const payload = sliceG711SamplePayload(item, fileData);
