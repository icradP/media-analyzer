import { buildAdtsFixedHeader7, getAacSamplingFrequencyIndex } from "../codec/aacAdts.js";
import { decodeG711ToFloat32 } from "../codec/g711.js";
import { normalizeVideoCodecLabel, videoCodecFamilyFromLabel } from "../codec/videoCodecLabels.js";
import { drawVideoFrameToCanvasContain } from "./canvasFrameRender.js";

function firstDefined(...vals) {
    for (const v of vals) {
        if (v !== undefined && v !== null) return v;
    }
    return null;
}

export function pickPrimaryMediaResult(mediaInfo) {
    if (!mediaInfo || typeof mediaInfo !== "object") return null;
    const looksLikeMediaResult = (v) =>
        !!v &&
        typeof v === "object" &&
        (Array.isArray(v.streams) || Array.isArray(v.frames) || !!v.formatSpecific || !!v.format);
    // Accept both shapes:
    // 1) mediaInfo map: { mp4: result, ts: result, ... }
    // 2) direct analysis result: { format, streams, frames, formatSpecific }
    if (looksLikeMediaResult(mediaInfo)) {
        return mediaInfo;
    }
    const named =
        mediaInfo.flv ||
        mediaInfo.mp4 ||
        mediaInfo.ts ||
        mediaInfo["mpeg-ts"] ||
        mediaInfo.mpegts ||
        mediaInfo.ps ||
        mediaInfo.rtp ||
        mediaInfo.mkv ||
        mediaInfo.h264 ||
        mediaInfo.h265 ||
        mediaInfo.mp3 ||
        mediaInfo.wav ||
        mediaInfo.flac ||
        mediaInfo.opus;
    if (looksLikeMediaResult(named)) return named;
    // Fallback for dynamic keys like "mpeg-ts", "unknown", etc.
    for (const v of Object.values(mediaInfo)) {
        if (looksLikeMediaResult(v)) return v;
    }
    return null;
}

export function detectVideoCodecForPlayback(mediaInfo) {
    const primary = pickPrimaryMediaResult(mediaInfo);
    if (!primary) return null;
    const streams = Array.isArray(primary.streams) ? primary.streams : [];
    const video = streams.find((s) => s.codecType === "video");
    return videoCodecFamilyFromLabel(video?.codecName);
}

export function collectVideoFrames(mediaInfo) {
    const primary = pickPrimaryMediaResult(mediaInfo);
    const frames = Array.isArray(primary?.frames) ? primary.frames : [];
    return frames.filter((f) => f.mediaType === "video");
}

export function collectAudioFrames(mediaInfo) {
    const primary = pickPrimaryMediaResult(mediaInfo);
    const frames = Array.isArray(primary?.frames) ? primary.frames : [];
    return frames.filter((f) => f.mediaType === "audio");
}

export function sliceFrameBytes(frame, fileData) {
    if (!frame) return null;
    const direct = firstDefined(
        frame._rawData,
        frame.formatSpecific?._rawData,
        frame._assembledESData,
        frame.formatSpecific?._assembledESData,
        frame.data instanceof Uint8Array ? frame.data : null,
    );
    if (direct instanceof Uint8Array && direct.length > 0) return direct;
    if (fileData instanceof Uint8Array) {
        // Prefer codec-specific data ranges set by container parsers (e.g. FLV).
        // For FLV the tag-level offset/size points at the FLV tag header rather
        // than the actual encoded payload, so we must use these field offsets to
        // get the real AVCC / HEVC / audio sample bytes the decoder expects.
        const fo = frame?.formatSpecific?.fieldOffsets || {};
        const codecRange =
            fo.avcData ||
            fo.hevcData ||
            fo.frameData ||
            fo.configData ||
            fo.videoPayload ||
            fo.audioData ||
            fo.aacData ||
            fo.data;
        if (
            codecRange &&
            Number.isFinite(codecRange.offset) &&
            Number.isFinite(codecRange.length) &&
            codecRange.length > 0
        ) {
            const start = Number(codecRange.offset);
            const end = start + Number(codecRange.length);
            if (start >= 0 && end <= fileData.length) {
                return fileData.subarray(start, end);
            }
        }
        const offset = firstDefined(
            frame.offset,
            frame.formatSpecific?.offset,
            frame.formatSpecific?._byteOffset,
            0,
        );
        const size = firstDefined(
            frame.size,
            frame.formatSpecific?.size,
            frame.formatSpecific?.dataSize,
            frame.formatSpecific?._byteLength,
            0,
        );
        if (Number.isFinite(offset) && Number.isFinite(size) && size > 0) {
            return fileData.subarray(offset, offset + size);
        }
    }
    return null;
}

function findAnnexBStartCode(data, from) {
    for (let i = from; i + 3 < data.length; i++) {
        if (data[i] === 0 && data[i + 1] === 0) {
            if (data[i + 2] === 1) return { offset: i, length: 3 };
            if (i + 3 < data.length && data[i + 2] === 0 && data[i + 3] === 1) return { offset: i, length: 4 };
        }
    }
    return null;
}

function splitAnnexBNalus(data) {
    const out = [];
    let pos = 0;
    while (pos < data.length) {
        const sc = findAnnexBStartCode(data, pos);
        if (!sc) break;
        const payloadStart = sc.offset + sc.length;
        const next = findAnnexBStartCode(data, payloadStart);
        const payloadEnd = next ? next.offset : data.length;
        if (payloadEnd > payloadStart) out.push(data.subarray(payloadStart, payloadEnd));
        pos = payloadEnd;
    }
    return out;
}

function hasAnnexBStartCode(data) {
    // Only treat the payload as Annex-B when it STARTS with a NAL start code.
    // AVCC slice data legitimately contains the byte sequence `00 00 01` inside
    // coded data; scanning the whole buffer would mis-classify AVCC as Annex-B
    // and lead to garbage NAL splits / lost SPS/PPS detection.
    if (!(data instanceof Uint8Array) || data.length < 3) return false;
    if (data[0] === 0 && data[1] === 0 && data[2] === 1) return true;
    if (data.length >= 4 && data[0] === 0 && data[1] === 0 && data[2] === 0 && data[3] === 1) return true;
    return false;
}

function isLikelyNaluHeader(nalu, codecFamily) {
    if (!(nalu instanceof Uint8Array) || nalu.length < 1) return false;
    if (codecFamily === "h264") {
        const forbiddenZeroBit = (nalu[0] & 0x80) === 0;
        const nalRefIdc = (nalu[0] >> 5) & 0x03;
        const nalType = nalu[0] & 0x1f;
        return forbiddenZeroBit && nalType >= 1 && nalType <= 12 && !(nalRefIdc === 0 && [5, 7, 8].includes(nalType));
    }
    if (codecFamily === "hevc") {
        if (nalu.length < 2) return false;
        const forbiddenZeroBit = (nalu[0] & 0x80) === 0;
        const nalType = (nalu[0] >> 1) & 0x3f;
        return forbiddenZeroBit && nalType >= 0 && nalType <= 40;
    }
    return false;
}

function looksLikeLengthPrefixedNalUnits(data, codecFamily, lengthSize = 4) {
    if (!(data instanceof Uint8Array) || data.length <= lengthSize) return false;
    let off = 0;
    let count = 0;
    while (off + lengthSize <= data.length) {
        let len = 0;
        for (let i = 0; i < lengthSize; i++) len = (len << 8) | data[off + i];
        off += lengthSize;
        if (len <= 0 || off + len > data.length) return false;
        const nalu = data.subarray(off, off + len);
        if (!isLikelyNaluHeader(nalu, codecFamily)) return false;
        count += 1;
        off += len;
    }
    return count > 0 && off === data.length;
}

function annexBToLengthPrefixed(data) {
    const nalus = splitAnnexBNalus(data);
    if (!nalus.length) return data;
    const total = nalus.reduce((n, nalu) => n + 4 + nalu.length, 0);
    const out = new Uint8Array(total);
    let off = 0;
    for (const nalu of nalus) {
        const len = nalu.length >>> 0;
        out[off] = (len >>> 24) & 0xff;
        out[off + 1] = (len >>> 16) & 0xff;
        out[off + 2] = (len >>> 8) & 0xff;
        out[off + 3] = len & 0xff;
        out.set(nalu, off + 4);
        off += 4 + nalu.length;
    }
    return out;
}

function parseParameterSetsFromAnnexB(data, codecFamily) {
    const nalus = splitAnnexBNalus(data);
    let sps = null;
    let pps = null;
    let vps = null;
    for (const nalu of nalus) {
        if (!nalu.length) continue;
        if (codecFamily === "h264") {
            const nalType = nalu[0] & 0x1f;
            if (nalType === 7 && !sps) sps = nalu.slice(0);
            else if (nalType === 8 && !pps) pps = nalu.slice(0);
        } else if (codecFamily === "hevc") {
            if (nalu.length < 2) continue;
            const nalType = (nalu[0] >> 1) & 0x3f;
            if (nalType === 32 && !vps) vps = nalu.slice(0);
            else if (nalType === 33 && !sps) sps = nalu.slice(0);
            else if (nalType === 34 && !pps) pps = nalu.slice(0);
        }
        if (codecFamily === "h264" && sps && pps) break;
        if (codecFamily === "hevc" && vps && sps && pps) break;
    }
    return { sps, pps, vps };
}

function parseParameterSetsFromLengthPrefixed(data, codecFamily) {
    if (!(data instanceof Uint8Array)) return { sps: null, pps: null, vps: null };
    let sps = null;
    let pps = null;
    let vps = null;
    let off = 0;
    while (off + 4 <= data.length) {
        const len =
            ((data[off] << 24) >>> 0) |
            (data[off + 1] << 16) |
            (data[off + 2] << 8) |
            data[off + 3];
        off += 4;
        if (len <= 0 || off + len > data.length) break;
        const nalu = data.subarray(off, off + len);
        off += len;
        if (!nalu.length) continue;
        if (codecFamily === "h264") {
            const nalType = nalu[0] & 0x1f;
            if (nalType === 7 && !sps) sps = nalu.slice(0);
            else if (nalType === 8 && !pps) pps = nalu.slice(0);
        } else if (codecFamily === "hevc") {
            if (nalu.length < 2) continue;
            const nalType = (nalu[0] >> 1) & 0x3f;
            if (nalType === 32 && !vps) vps = nalu.slice(0);
            else if (nalType === 33 && !sps) sps = nalu.slice(0);
            else if (nalType === 34 && !pps) pps = nalu.slice(0);
        }
        if (codecFamily === "h264" && sps && pps) break;
        if (codecFamily === "hevc" && vps && sps && pps) break;
    }
    return { sps, pps, vps };
}

function extractParameterSetsFromMp4Boxes(primaryResult, codecFamily) {
    const boxes = primaryResult?.formatSpecific?.boxes;
    if (!Array.isArray(boxes)) return { sps: null, pps: null, vps: null };
    const stack = [...boxes];
    while (stack.length > 0) {
        const box = stack.pop();
        if (Array.isArray(box?.children)) {
            for (const child of box.children) stack.push(child);
        }
        if (box?.type !== "stsd") continue;
        const entries = box?.data?.entries || box?.children || [];
        for (const entry of entries) {
            const cfg = entry?.config || entry?.data || null;
            if (!cfg) continue;
            if (codecFamily === "h264") {
                const sps = asUint8Array(cfg?.sps?.[0]);
                const pps = asUint8Array(cfg?.pps?.[0]);
                if (sps && pps) return { sps, pps, vps: null };
            } else if (codecFamily === "hevc") {
                const vps = asUint8Array(cfg?.vps?.[0]);
                const sps = asUint8Array(cfg?.sps?.[0]);
                const pps = asUint8Array(cfg?.pps?.[0]);
                if (vps && sps && pps) return { sps, pps, vps };
            }
        }
    }
    return { sps: null, pps: null, vps: null };
}

function extractParameterSetsFromFlvSequenceHeader(primaryResult, codecFamily) {
    const frames = Array.isArray(primaryResult?.frames) ? primaryResult.frames : [];
    const fileData = primaryResult?.formatSpecific?.fileData;
    if (!(fileData instanceof Uint8Array)) return { sps: null, pps: null, vps: null };
    for (const frame of frames) {
        const fs = frame?.formatSpecific;
        if (!fs?.sequenceHeader) continue;
        const seq = fs.sequenceHeader;
        const fieldOffsets = fs.fieldOffsets || {};

        const readNaluByOffsetKey = (offsetKey, itemKey) => {
            const pos = fieldOffsets[offsetKey];
            const item = seq?.[itemKey];
            const naluLength = item?.naluLength;
            if (!pos || !Number.isFinite(naluLength)) return null;
            const start = Number(pos.offset) + 2;
            const end = start + naluLength;
            if (!Number.isFinite(start) || !Number.isFinite(end)) return null;
            if (start < 0 || end > fileData.length) return null;
            return fileData.slice(start, end);
        };

        if (codecFamily === "h264") {
            const sps = readNaluByOffsetKey("sequenceHeader.sps[0].naluLength", "sps[0]");
            const pps = readNaluByOffsetKey("sequenceHeader.pps[0].naluLength", "pps[0]");
            if (sps && pps) return { sps, pps, vps: null };
        } else if (codecFamily === "hevc") {
            const vps = readNaluByOffsetKey("sequenceHeader.vps[0].naluLength", "vps[0]");
            const sps = readNaluByOffsetKey("sequenceHeader.sps[0].naluLength", "sps[0]");
            const pps = readNaluByOffsetKey("sequenceHeader.pps[0].naluLength", "pps[0]");
            if (vps && sps && pps) return { sps, pps, vps };
        }
    }
    return { sps: null, pps: null, vps: null };
}

function extractFlvDecoderConfigDescription(primaryResult, codecFamily) {
    const frames = Array.isArray(primaryResult?.frames) ? primaryResult.frames : [];
    const fileData = primaryResult?.formatSpecific?.fileData;
    if (!(fileData instanceof Uint8Array)) return null;
    for (const frame of frames) {
        const fs = frame?.formatSpecific;
        if (!fs?.sequenceHeader) continue;
        const fo = fs.fieldOffsets || {};
        const range =
            (codecFamily === "h264" && fo.avcData) ||
            (codecFamily === "hevc" && (fo.hevcData || fo.configData)) ||
            null;
        if (!range) continue;
        const start = Number(range.offset);
        const length = Number(range.length);
        if (!Number.isFinite(start) || !Number.isFinite(length) || length <= 0) continue;
        if (start < 0 || start + length > fileData.length) continue;
        return fileData.slice(start, start + length);
    }
    return null;
}

function asUint8Array(v) {
    if (!v) return null;
    if (v instanceof Uint8Array) return v;
    if (ArrayBuffer.isView(v)) {
        return new Uint8Array(v.buffer, v.byteOffset, v.byteLength);
    }
    if (v instanceof ArrayBuffer) return new Uint8Array(v);
    if (Array.isArray(v)) return Uint8Array.from(v);
    if (typeof v === "object") {
        const keys = Object.keys(v)
            .filter((k) => /^\d+$/.test(k))
            .map((k) => Number(k))
            .sort((a, b) => a - b);
        if (keys.length > 0) {
            const arr = new Uint8Array(keys.length);
            for (let i = 0; i < keys.length; i++) {
                arr[i] = Number(v[String(keys[i])]) & 0xff;
            }
            return arr;
        }
    }
    return null;
}

function extractParameterSetsFromStreamConfig(primaryResult, codecFamily) {
    const stream = (primaryResult?.streams || []).find((s) => s.codecType === "video");
    const cfg = stream?.decoderConfig;
    if (!cfg || typeof cfg !== "object") return { sps: null, pps: null, vps: null };
    if (codecFamily === "h264") {
        const sps = asUint8Array(cfg?.sps?.[0]);
        const pps = asUint8Array(cfg?.pps?.[0]);
        return { sps, pps, vps: null };
    }
    if (codecFamily === "hevc") {
        const vps = asUint8Array(cfg?.vps?.[0]);
        const sps = asUint8Array(cfg?.sps?.[0]);
        const pps = asUint8Array(cfg?.pps?.[0]);
        return { sps, pps, vps };
    }
    return { sps: null, pps: null, vps: null };
}

function buildAvcDecoderConfigRecord(sps, pps) {
    if (!(sps instanceof Uint8Array) || !(pps instanceof Uint8Array) || sps.length < 4) return null;
    const out = new Uint8Array(11 + sps.length + pps.length);
    let o = 0;
    out[o++] = 1;
    out[o++] = sps[1];
    out[o++] = sps[2];
    out[o++] = sps[3];
    out[o++] = 0xff;
    out[o++] = 0xe1;
    out[o++] = (sps.length >> 8) & 0xff;
    out[o++] = sps.length & 0xff;
    out.set(sps, o);
    o += sps.length;
    out[o++] = 1;
    out[o++] = (pps.length >> 8) & 0xff;
    out[o++] = pps.length & 0xff;
    out.set(pps, o);
    return out;
}

function buildHevcDecoderConfigRecord(vps, sps, pps) {
    if (!(vps instanceof Uint8Array) || !(sps instanceof Uint8Array) || !(pps instanceof Uint8Array)) return null;
    const total = 26 + (5 + vps.length) + (5 + sps.length) + (5 + pps.length);
    const out = new Uint8Array(total);
    let o = 0;
    out[o++] = 1;
    out[o++] = 0;
    out[o++] = 0;
    out[o++] = 0;
    out[o++] = 0;
    out[o++] = 0;
    out[o++] = 0x90;
    out[o++] = 0;
    out[o++] = 0;
    out[o++] = 0;
    out[o++] = 0;
    out[o++] = 0;
    out[o++] = 0x5d;
    out[o++] = 0xf0;
    out[o++] = 0;
    out[o++] = 0xfc;
    out[o++] = 0xfd;
    out[o++] = 0xf8;
    out[o++] = 0xf8;
    out[o++] = 0;
    out[o++] = 0;
    out[o++] = 0x0f;
    out[o++] = 3;
    out[o++] = 32;
    out[o++] = 0;
    out[o++] = 1;
    out[o++] = (vps.length >> 8) & 0xff;
    out[o++] = vps.length & 0xff;
    out.set(vps, o);
    o += vps.length;
    out[o++] = 33;
    out[o++] = 0;
    out[o++] = 1;
    out[o++] = (sps.length >> 8) & 0xff;
    out[o++] = sps.length & 0xff;
    out.set(sps, o);
    o += sps.length;
    out[o++] = 34;
    out[o++] = 0;
    out[o++] = 1;
    out[o++] = (pps.length >> 8) & 0xff;
    out[o++] = pps.length & 0xff;
    out.set(pps, o);
    return out;
}

function isVideoFrameKeyframe(frame) {
    if (!frame) return false;
    if (frame.pictureType === "I" || frame.pictureType === "IDR") return true;
    if (frame.isKeyframe === true || frame.isKeyFrame === true) return true;
    const fs = frame.formatSpecific || {};
    return fs.keyframe === true || fs._frameType_value === 1;
}

function resolveAudioTrack(mediaInfo) {
    const primary = pickPrimaryMediaResult(mediaInfo);
    const streams = Array.isArray(primary?.streams) ? primary.streams : [];
    return streams.find((s) => s.codecType === "audio") || null;
}

function wrapRawAacToAdts(payload, mediaInfo) {
    const audioTrack = resolveAudioTrack(mediaInfo);
    const objectType = Number(audioTrack?.profile || 2);
    const sampleRate = Number(audioTrack?.sampleRate || 44100);
    const channelConfig = Number(audioTrack?.channels || 2);
    const sampleRateIndex = getAacSamplingFrequencyIndex(sampleRate);
    // buildAdtsFixedHeader7(aacFrameLength, profile, samplingFreqIndex, channelConfig)
    const header = buildAdtsFixedHeader7(payload.length, objectType, sampleRateIndex, channelConfig);
    const out = new Uint8Array(header.length + payload.length);
    out.set(header, 0);
    out.set(payload, header.length);
    return out;
}

function bytesToNumberArray(bytes) {
    return bytes instanceof Uint8Array ? Array.from(bytes) : [];
}

function bytesToHexArray(bytes) {
    return bytes instanceof Uint8Array
        ? Array.from(bytes, (b) => b.toString(16).toUpperCase().padStart(2, "0"))
        : [];
}

function bytesToBase64(bytes) {
    if (!(bytes instanceof Uint8Array)) return "";
    let s = "";
    const chunk = 0x8000;
    for (let i = 0; i < bytes.length; i += chunk) {
        s += String.fromCharCode(...bytes.subarray(i, i + chunk));
    }
    return btoa(s);
}

function describeDecoderDescription(description) {
    if (!(description instanceof Uint8Array)) {
        return {
            length: 0,
            bytes: [],
            hex: [],
            base64: "",
        };
    }
    return {
        length: description.length,
        bytes: bytesToNumberArray(description),
        hex: bytesToHexArray(description),
        base64: bytesToBase64(description),
    };
}

function h264NalTypeLabel(type) {
    return ({
        1: "non-IDR slice",
        5: "IDR slice",
        6: "SEI",
        7: "SPS",
        8: "PPS",
        9: "AUD",
    })[type] || `type ${type}`;
}

function lengthSizeFromDecoderDescription(description) {
    if (description instanceof Uint8Array && description.length >= 5) {
        return (description[4] & 0x03) + 1;
    }
    return 4;
}

function describeLengthPrefixedNalus(bytes, description) {
    if (!(bytes instanceof Uint8Array) || bytes.length === 0) return [];
    const lengthSize = lengthSizeFromDecoderDescription(description);
    const nalus = [];
    let off = 0;
    while (off + lengthSize <= bytes.length) {
        let len = 0;
        for (let i = 0; i < lengthSize; i++) len = (len << 8) | bytes[off + i];
        const lengthOffset = off;
        off += lengthSize;
        if (len <= 0 || off + len > bytes.length) {
            nalus.push({
                error: "invalid length-prefixed NAL",
                lengthOffset,
                declaredLength: len,
                remainingBytes: bytes.length - off,
            });
            break;
        }
        const nalHeader = bytes[off];
        const nalType = nalHeader & 0x1f;
        nalus.push({
            offset: off,
            length: len,
            nalHeader,
            nalType,
            nalTypeName: h264NalTypeLabel(nalType),
        });
        off += len;
    }
    if (off !== bytes.length) {
        nalus.push({
            error: "trailing bytes",
            offset: off,
            length: bytes.length - off,
        });
    }
    return nalus;
}

function describeAnnexBNalus(bytes) {
    if (!(bytes instanceof Uint8Array) || bytes.length === 0) return [];
    return splitAnnexBNalus(bytes).map((nalu, idx) => {
        const nalHeader = nalu[0];
        const nalType = nalHeader & 0x1f;
        return {
            index: idx,
            length: nalu.length,
            nalHeader,
            nalType,
            nalTypeName: h264NalTypeLabel(nalType),
        };
    });
}

function describeEncodedChunkBytes(bytes, description) {
    if (!(bytes instanceof Uint8Array)) {
        return {
            size: 0,
            firstBytes: [],
            lastBytes: [],
            nalus: [],
        };
    }
    const annexB = hasAnnexBStartCode(bytes);
    return {
        size: bytes.length,
        bitstream: annexB ? "annexb" : "length-prefixed",
        firstBytes: bytesToHexArray(bytes.subarray(0, Math.min(32, bytes.length))),
        lastBytes: bytesToHexArray(bytes.subarray(Math.max(0, bytes.length - 16))),
        nalus: annexB ? describeAnnexBNalus(bytes) : describeLengthPrefixedNalus(bytes, description),
    };
}

function describeEncodedFrameSummaries(encodedFrames, frameMeta) {
    return (Array.isArray(encodedFrames) ? encodedFrames : []).map((item, i) => {
        const meta = Array.isArray(frameMeta) ? frameMeta[i] || null : null;
        const data = item?.data instanceof Uint8Array ? item.data : null;
        return {
            chunkIndex: i,
            frameIndex: meta?.frameIndex ?? null,
            type: item?.type === "key" ? "key" : "delta",
            size: data?.length || 0,
            pts: meta?.pts ?? null,
            dts: meta?.dts ?? null,
            ptsTime: meta?.ptsTime ?? null,
            dtsTime: meta?.dtsTime ?? null,
        };
    });
}

function concatUint8Arrays(chunks) {
    const total = chunks.reduce((n, chunk) => n + (chunk?.length || 0), 0);
    if (!total) return null;
    const out = new Uint8Array(total);
    let offset = 0;
    for (const chunk of chunks) {
        if (!(chunk instanceof Uint8Array) || chunk.length === 0) continue;
        out.set(chunk, offset);
        offset += chunk.length;
    }
    return offset > 0 ? out : null;
}

function frameCodecNameHint(frame, mediaInfo) {
    const trackName = String(resolveAudioTrack(mediaInfo)?.codecName || "").toLowerCase();
    if (trackName) return trackName;
    const fs = frame?.formatSpecific || {};
    const fromFrame = String(frame?.codecName || fs?.codecName || fs?.codec || fs?.soundFormat || "").toLowerCase();
    return fromFrame || "";
}

function isG711CodecName(codecName) {
    return codecName.includes("g.711") || codecName.includes("g711") || codecName.includes("pcma") || codecName.includes("pcmu");
}

function isG711AudioFrame(frame, mediaInfo) {
    const codecName = frameCodecNameHint(frame, mediaInfo);
    if (isG711CodecName(codecName)) return true;
    const soundFormat = Number(frame?.formatSpecific?._soundFormat_value);
    return soundFormat === 7 || soundFormat === 8;
}

function g711LawForFrame(frame, mediaInfo) {
    const codecName = frameCodecNameHint(frame, mediaInfo);
    if (codecName.includes("mu-law") || codecName.includes("mulaw") || codecName.includes("u-law") || codecName.includes("ulaw") || codecName.includes("pcmu")) return "mulaw";
    if (codecName.includes("a-law") || codecName.includes("alaw") || codecName.includes("pcma")) return "alaw";
    const soundFormat = Number(frame?.formatSpecific?._soundFormat_value);
    if (soundFormat === 8) return "mulaw";
    return "alaw";
}

function resolveAudioChannelCount(frame, mediaInfo) {
    const trackChannels = Number(resolveAudioTrack(mediaInfo)?.channels);
    if (Number.isFinite(trackChannels) && trackChannels > 0) return trackChannels;
    const fs = frame?.formatSpecific || {};
    const soundType = Number(fs?._soundType_value);
    if (Number.isFinite(soundType)) return soundType === 1 ? 2 : 1;
    return 1;
}

function resolveG711SampleRate(frame, mediaInfo) {
    const trackRate = Number(resolveAudioTrack(mediaInfo)?.sampleRate);
    if (Number.isFinite(trackRate) && trackRate > 0) return trackRate;
    const fs = frame?.formatSpecific || {};
    const soundRate = Number(fs?._soundRate_value);
    if (soundRate === 0) return 5512;
    if (soundRate === 1) return 11025;
    if (soundRate === 2) return 22050;
    if (soundRate === 3) return 44100;
    return 8000;
}

function buildG711AudioBuffer(ctx, frame, mediaInfo, options = {}) {
    const frames = findAudioFrameWindowByIndex(
        frame,
        mediaInfo,
        Number(options.maxFrames) > 0 ? Number(options.maxFrames) : 1,
    );
    return buildG711AudioBufferFromFrames(ctx, frames, mediaInfo);
}

function buildG711AudioBufferFromFrames(ctx, frames, mediaInfo) {
    const inputFrames = Array.isArray(frames) ? frames : [];
    const primary = pickPrimaryMediaResult(mediaInfo);
    const fileData = primary?.formatSpecific?.fileData;
    const chunks = [];
    for (const item of inputFrames) {
        const payload = sliceFrameBytes(item, fileData);
        if (payload instanceof Uint8Array && payload.length > 0) chunks.push(payload);
    }
    const bytes = concatUint8Arrays(chunks);
    if (!(bytes instanceof Uint8Array) || bytes.length === 0) return null;
    const seedFrame = inputFrames[0] || null;
    const channels = resolveAudioChannelCount(seedFrame, mediaInfo);
    const sampleRate = resolveG711SampleRate(seedFrame, mediaInfo);
    const decoded = decodeG711ToFloat32(bytes, g711LawForFrame(seedFrame, mediaInfo), channels);
    if (!decoded.length || decoded[0].length === 0) return null;
    const buffer = ctx.createBuffer(decoded.length, decoded[0].length, sampleRate);
    for (let ch = 0; ch < decoded.length; ch++) {
        buffer.copyToChannel(decoded[ch], ch);
    }
    return buffer;
}

function findAudioFrameWindowByIndex(frame, mediaInfo, maxFrames) {
    const frames = collectAudioFrames(mediaInfo);
    if (!Array.isArray(frames) || frames.length === 0) return [];
    const selectedIndex = Number(frame?.index);
    const start = Number.isFinite(selectedIndex)
        ? frames.findIndex((f) => Number(f?.index) === selectedIndex)
        : -1;
    if (start < 0) return [frame].filter(Boolean);
    return frames.slice(start, Math.min(frames.length, start + Math.max(1, maxFrames)));
}

function normalizeAudioPayloadForPlayback(frame, mediaInfo, payload, options = {}) {
    if (!(payload instanceof Uint8Array) || payload.length === 0) return null;
    const codecName = frameCodecNameHint(frame, mediaInfo);
    const isAdtsLike = payload.length >= 2 && payload[0] === 0xff && (payload[1] & 0xf0) === 0xf0;
    const isRawAac = (codecName.includes("aac") || codecName.includes("mp4a")) && !isAdtsLike;
    const adtsMode = options?.aacAdtsMode || "auto";
    if (!isRawAac) return payload;
    if (adtsMode === "off") return payload;
    return wrapRawAacToAdts(payload, mediaInfo);
}

export function buildAudioPlaybackBytesForFrameRange(frames, mediaInfo, options = {}) {
    const primary = pickPrimaryMediaResult(mediaInfo);
    const inputFrames = Array.isArray(frames) ? frames : [];
    const out = [];
    for (const item of inputFrames) {
        const payload = sliceFrameBytes(item, primary?.formatSpecific?.fileData);
        const normalized = normalizeAudioPayloadForPlayback(item, mediaInfo, payload, options);
        if (normalized) out.push(normalized);
    }
    return concatUint8Arrays(out);
}

export function buildAudioPlaybackBytes(frame, mediaInfo, options = {}) {
    const windowFrames = findAudioFrameWindowByIndex(
        frame,
        mediaInfo,
        Number(options.maxFrames) > 0 ? Number(options.maxFrames) : 1,
    );
    return buildAudioPlaybackBytesForFrameRange(windowFrames, mediaInfo, options);
}

export async function decodeVideoFramesToCanvas({
    encodedFrames,
    frameMeta = null,
    codecString,
    description = null,
    canvas,
    targetFrameIndex = null,
    timestampStepUs = 33333,
    optimizeForLatency = true,
    returnVideoFrame = false,
}) {
    if (typeof VideoDecoder === "undefined") {
        throw new Error("VideoDecoder API is not available in this browser.");
    }
    if (!canvas) throw new Error("canvas is required.");
    if (!Array.isArray(encodedFrames) || encodedFrames.length === 0) {
        throw new Error("encodedFrames is empty.");
    }
    const shouldSelectTargetFrame = targetFrameIndex !== null && targetFrameIndex !== undefined;
    const targetMetaIndex = shouldSelectTargetFrame && Array.isArray(frameMeta)
        ? frameMeta.findIndex((m) => Number(m?.frameIndex) === Number(targetFrameIndex))
        : -1;
    const targetTimestampUs = targetMetaIndex >= 0 ? targetMetaIndex * timestampStepUs : null;
    let fallbackFrame = null;
    let targetFrame = null;
    let decodeError = null;
    let decodeErrorAt = null;
    let lastQueuedChunk = null;
    const closeHeldFrames = () => {
        if (targetFrame) {
            try {
                targetFrame.close();
            } catch {
                // ignore close error
            }
            targetFrame = null;
        }
        if (fallbackFrame) {
            try {
                fallbackFrame.close();
            } catch {
                // ignore close error
            }
            fallbackFrame = null;
        }
    };
    const closeDecoder = () => {
        try {
            decoder.close();
        } catch {
            // ignore close error
        }
    };
    const drawFrameToCanvas = (frameToDraw) => {
        if (!frameToDraw) return false;
        drawVideoFrameToCanvasContain(frameToDraw, canvas, { fitTo: "parent" });
        return true;
    };
    const finishWithFrame = (frameToDraw) => {
        try {
            const drawn = drawFrameToCanvas(frameToDraw);
            const frame = returnVideoFrame && drawn && typeof frameToDraw?.clone === "function"
                ? frameToDraw.clone()
                : null;
            return { drawn, frame };
        } finally {
            closeHeldFrames();
            closeDecoder();
        }
    };
    const yieldDecoderOutput = () => new Promise((resolve) => setTimeout(resolve, 0));
    const decoder = new VideoDecoder({
        output: (frame) => {
            if (targetTimestampUs !== null && frame.timestamp === targetTimestampUs) {
                if (targetFrame) {
                    try {
                        targetFrame.close();
                    } catch {
                        // ignore close error
                    }
                }
                targetFrame = frame;
                return;
            }
            if (fallbackFrame) {
                try {
                    fallbackFrame.close();
                } catch {
                    // ignore close error
                }
            }
            fallbackFrame = frame;
        },
        error: (err) => {
            decodeError = err || new Error("VideoDecoder error");
            decodeErrorAt = lastQueuedChunk;
        },
    });
    decoder.configure({
        codec: codecString,
        description: description || undefined,
        optimizeForLatency,
    });
    const descriptionDiagnostics = describeDecoderDescription(description);
    const encodedFrameSummaries = describeEncodedFrameSummaries(encodedFrames, frameMeta);
    let ts = 0;
    for (let i = 0; i < encodedFrames.length; i++) {
        const item = encodedFrames[i];
        const data = item?.data;
        if (!(data instanceof Uint8Array) || data.length === 0) continue;
        const chunkType = item?.type === "key" ? "key" : "delta";
        const meta = Array.isArray(frameMeta) ? frameMeta[i] || null : null;
        lastQueuedChunk = {
            chunkIndex: i,
            type: chunkType,
            timestampUs: ts,
            size: data.length,
            frameIndex: meta?.frameIndex ?? null,
            pts: meta?.pts ?? null,
            dts: meta?.dts ?? null,
            ptsTime: meta?.ptsTime ?? null,
            dtsTime: meta?.dtsTime ?? null,
        };
        const chunk = new EncodedVideoChunk({
            type: chunkType,
            timestamp: ts,
            data,
        });
        try {
            decoder.decode(chunk);
        } catch (err) {
            if (targetFrame) {
                return finishWithFrame(targetFrame);
            }
            const wrapped = new Error(`VideoDecoder.decode failed at chunk ${i}: ${err?.message || String(err)}`);
            wrapped.cause = err;
            wrapped.diagnostics = {
                stage: "decode",
                codecString,
                hasDescription: !!description,
                descriptionLength: description ? description.length : 0,
                description: descriptionDiagnostics,
                chunk: lastQueuedChunk,
                chunkData: describeEncodedChunkBytes(data, description),
                encodedFrames: encodedFrameSummaries,
            };
            closeDecoder();
            closeHeldFrames();
            throw wrapped;
        }
        if (targetTimestampUs !== null && ts >= targetTimestampUs && !targetFrame) {
            await yieldDecoderOutput();
        }
        ts += timestampStepUs;
    }
    try {
        await decoder.flush();
    } catch (err) {
        if (targetFrame) {
            return finishWithFrame(targetFrame);
        }
        const wrapped = new Error(`VideoDecoder.flush failed: ${err?.message || String(err)}`);
        wrapped.cause = err;
        wrapped.diagnostics = {
            stage: "flush",
            codecString,
            hasDescription: !!description,
            descriptionLength: description ? description.length : 0,
            description: descriptionDiagnostics,
            chunk: decodeErrorAt || lastQueuedChunk,
            chunkData: describeEncodedChunkBytes(
                encodedFrames[(decodeErrorAt || lastQueuedChunk)?.chunkIndex]?.data,
                description,
            ),
            encodedFrames: encodedFrameSummaries,
        };
        closeDecoder();
        closeHeldFrames();
        throw wrapped;
    }
    if (decodeError) {
        if (targetFrame) {
            return finishWithFrame(targetFrame);
        }
        const wrapped = new Error(`VideoDecoder error callback: ${decodeError?.message || String(decodeError)}`);
        wrapped.cause = decodeError;
        wrapped.diagnostics = {
            stage: "decoder-error-callback",
            codecString,
            hasDescription: !!description,
            descriptionLength: description ? description.length : 0,
            description: descriptionDiagnostics,
            chunk: decodeErrorAt || lastQueuedChunk,
            chunkData: describeEncodedChunkBytes(
                encodedFrames[(decodeErrorAt || lastQueuedChunk)?.chunkIndex]?.data,
                description,
            ),
            encodedFrames: encodedFrameSummaries,
        };
        closeDecoder();
        closeHeldFrames();
        throw wrapped;
    }
    const frameToDraw = targetFrame || fallbackFrame;
    return finishWithFrame(frameToDraw);
}

export function buildVideoDecodePlan({
    mediaInfo,
    targetFrameIndex,
}) {
    const primary = pickPrimaryMediaResult(mediaInfo);
    if (!primary) throw new Error("No media result.");
    const stream = (primary.streams || []).find((s) => s.codecType === "video");
    const codecLabel = normalizeVideoCodecLabel(stream?.codecName) || stream?.codecName || "unknown";
    const codecFamily = videoCodecFamilyFromLabel(codecLabel);
    if (!codecFamily) throw new Error(`Unsupported video codec: ${codecLabel}`);
    const isTsInput =
        String(primary?.format?.formatName || "").toLowerCase() === "mpeg-ts" ||
        String(primary?.format?.formatLongName || "").toLowerCase().includes("transport stream");
    const isFlvInput =
        String(primary?.format?.formatName || "").toLowerCase() === "flv" ||
        String(primary?.format?.formatLongName || "").toLowerCase().includes("flv");
    const isMp4Input =
        String(primary?.format?.formatName || "").toLowerCase().includes("mp4") ||
        String(primary?.format?.formatLongName || "").toLowerCase().includes("mp4");
    const isLengthPrefixedContainer = isFlvInput || isMp4Input;
    const isFlvVideoConfigFrame = (frame) => {
        if (!isFlvInput) return false;
        const fs = frame?.formatSpecific || {};
        if (Number(fs?._avcPacketType_value) === 0) return true;
        if (Number(fs?._hevcPacketType_value) === 0) return true;
        if (Number(fs?._isExHeader_value) === 1 && Number(fs?._packetType_value) === 0) return true;
        return false;
    };
    const videoFrames = collectVideoFrames(mediaInfo);
    if (!videoFrames.length) throw new Error("No video frames.");
    let idx = videoFrames.findIndex((f) => f.index === targetFrameIndex);
    if (idx < 0) throw new Error("Target frame not found in video frames.");
    // If the user-selected frame itself is a FLV config-only tag (sequence
    // header), advance forward to the next real decodable frame.
    if (isFlvVideoConfigFrame(videoFrames[idx])) {
        let probe = idx + 1;
        while (probe < videoFrames.length && isFlvVideoConfigFrame(videoFrames[probe])) probe += 1;
        if (probe >= videoFrames.length) {
            throw new Error("Target frame is a FLV sequence header without any subsequent decodable picture.");
        }
        idx = probe;
    }
    // Walk back to a real (decodable) keyframe — skipping FLV sequence header
    // tags whose isKeyframe metadata is `true` but which carry the
    // AVCDecoderConfigurationRecord rather than a coded picture.
    let keyIdx = idx;
    while (keyIdx >= 0) {
        const candidate = videoFrames[keyIdx];
        if (isVideoFrameKeyframe(candidate) && !isFlvVideoConfigFrame(candidate)) break;
        keyIdx -= 1;
    }
    if (keyIdx < 0) throw new Error("No previous keyframe found.");
    const windowFrames = videoFrames.slice(keyIdx, idx + 1);
    const fileData = primary.formatSpecific?.fileData;
    let hasAnnexBInput = false;
    let hasAnnexBOutput = false;
    const encodedFrames = [];
    const encodedFrameMeta = [];
    let sps = null;
    let pps = null;
    let vps = null;
    const paramSource = {
        window: false,
        flvSequenceHeader: false,
        streamDecoderConfig: false,
        mp4Boxes: false,
    };
    for (let i = 0; i < windowFrames.length; i++) {
        const frame = windowFrames[i];
        // FLV sequence header tags carry the DecoderConfigurationRecord, not a
        // coded picture; skip them so they aren't pushed as decoder chunks.
        if (isFlvVideoConfigFrame(frame)) continue;
        const payload = sliceFrameBytes(frame, fileData);
        if (!(payload instanceof Uint8Array) || payload.length === 0) continue;
        const looksLengthPrefixed = isLengthPrefixedContainer && looksLikeLengthPrefixedNalUnits(payload, codecFamily, 4);
        const usesAnnexB = hasAnnexBStartCode(payload) && !looksLengthPrefixed;
        if (usesAnnexB) hasAnnexBInput = true;
        if (usesAnnexB) {
            const ps = parseParameterSetsFromAnnexB(payload, codecFamily);
            if (ps.sps || ps.pps || ps.vps) paramSource.window = true;
            sps = sps || ps.sps;
            pps = pps || ps.pps;
            vps = vps || ps.vps;
        } else {
            const ps = parseParameterSetsFromLengthPrefixed(payload, codecFamily);
            if (ps.sps || ps.pps || ps.vps) paramSource.window = true;
            sps = sps || ps.sps;
            pps = pps || ps.pps;
            vps = vps || ps.vps;
        }
        const outputUsesAnnexB = usesAnnexB && isTsInput;
        if (outputUsesAnnexB) hasAnnexBOutput = true;
        encodedFrames.push({
            type: "delta",
            // TS demux outputs Annex-B AU data; keep it as Annex-B for WebCodecs.
            data: outputUsesAnnexB ? payload : usesAnnexB ? annexBToLengthPrefixed(payload) : payload,
        });
        encodedFrameMeta.push({
            frameIndex: frame?.index ?? null,
            pts: Number.isFinite(frame?.pts) ? frame.pts : null,
            dts: Number.isFinite(frame?.dts) ? frame.dts : null,
            ptsTime: Number.isFinite(frame?.ptsTime) ? frame.ptsTime : null,
            dtsTime: Number.isFinite(frame?.dtsTime) ? frame.dtsTime : null,
        });
    }
    if (!encodedFrames.length) throw new Error("No decodable frame payload in selected window.");
    // WebCodecs hard requirement: first chunk after configure/flush must be key.
    encodedFrames[0].type = "key";
    if (codecFamily === "h264" && (!sps || !pps)) {
        const fromFlvSeq = extractParameterSetsFromFlvSequenceHeader(primary, codecFamily);
        if (fromFlvSeq.sps || fromFlvSeq.pps || fromFlvSeq.vps) paramSource.flvSequenceHeader = true;
        sps = sps || fromFlvSeq.sps;
        pps = pps || fromFlvSeq.pps;
    }
    if (codecFamily === "h264" && (!sps || !pps)) {
        const fromStream = extractParameterSetsFromStreamConfig(primary, codecFamily);
        if (fromStream.sps || fromStream.pps || fromStream.vps) paramSource.streamDecoderConfig = true;
        sps = sps || fromStream.sps;
        pps = pps || fromStream.pps;
    }
    if (codecFamily === "h264" && (!sps || !pps)) {
        const fromBoxes = extractParameterSetsFromMp4Boxes(primary, codecFamily);
        if (fromBoxes.sps || fromBoxes.pps || fromBoxes.vps) paramSource.mp4Boxes = true;
        sps = sps || fromBoxes.sps;
        pps = pps || fromBoxes.pps;
    } else if (codecFamily === "hevc" && (!vps || !sps || !pps)) {
        const fromFlvSeq = extractParameterSetsFromFlvSequenceHeader(primary, codecFamily);
        if (fromFlvSeq.sps || fromFlvSeq.pps || fromFlvSeq.vps) paramSource.flvSequenceHeader = true;
        vps = vps || fromFlvSeq.vps;
        sps = sps || fromFlvSeq.sps;
        pps = pps || fromFlvSeq.pps;
    }
    if (codecFamily === "hevc" && (!vps || !sps || !pps)) {
        const fromStream = extractParameterSetsFromStreamConfig(primary, codecFamily);
        if (fromStream.sps || fromStream.pps || fromStream.vps) paramSource.streamDecoderConfig = true;
        vps = vps || fromStream.vps;
        sps = sps || fromStream.sps;
        pps = pps || fromStream.pps;
    }
    if (codecFamily === "hevc" && (!vps || !sps || !pps)) {
        const fromBoxes = extractParameterSetsFromMp4Boxes(primary, codecFamily);
        if (fromBoxes.sps || fromBoxes.pps || fromBoxes.vps) paramSource.mp4Boxes = true;
        vps = vps || fromBoxes.vps;
        sps = sps || fromBoxes.sps;
        pps = pps || fromBoxes.pps;
    }
    let description =
        codecFamily === "h264"
            ? buildAvcDecoderConfigRecord(sps, pps)
            : buildHevcDecoderConfigRecord(vps, sps, pps);
    if (!description && isFlvInput) {
        // Last-resort: FLV's sequence header `avcData/hevcData/configData`
        // bytes ARE the AVC/HEVC DecoderConfigurationRecord — usable directly
        // as the WebCodecs `description` field.
        const flvDirect = extractFlvDecoderConfigDescription(primary, codecFamily);
        if (flvDirect instanceof Uint8Array && flvDirect.length > 0) {
            description = flvDirect;
            paramSource.flvSequenceHeader = true;
        }
    }
    const effectiveDescription = hasAnnexBOutput ? null : description;
    if (codecFamily === "hevc" && !effectiveDescription) {
        const err = new Error("HEVC decoder description is missing (VPS/SPS/PPS not found).");
        err.diagnostics = {
            codecFamily,
            paramSource,
            hasVps: !!vps,
            hasSps: !!sps,
            hasPps: !!pps,
            windowFrameCount: windowFrames.length,
            encodedFrameCount: encodedFrames.length,
            decodeWindowStartFrameIndex: windowFrames[0]?.index ?? null,
            decodeWindowEndFrameIndex: windowFrames[windowFrames.length - 1]?.index ?? null,
        };
        throw err;
    }
    return {
        codecFamily,
        encodedFrames,
        encodedFrameMeta,
        description: effectiveDescription || null,
        decodeWindowStartFrameIndex: windowFrames[0]?.index ?? null,
        decodeWindowEndFrameIndex: windowFrames[windowFrames.length - 1]?.index ?? null,
        targetFrameIndex: videoFrames[idx]?.index ?? targetFrameIndex,
        hasAnnexBInput,
        hasAnnexBOutput,
        bitstreamFormat: hasAnnexBOutput ? "annexb" : "avc",
        hasParameterSetsInSamples: !!paramSource.window,
    };
}

export async function playAudioFrameWithWebAudio({
    frame,
    mediaInfo,
    audioContext = null,
    maxDecodeFrames = 10,
}) {
    const Ctx = globalThis.AudioContext || globalThis.webkitAudioContext;
    if (!Ctx) throw new Error("WebAudio API is not available in this browser.");
    const ctx = audioContext || new Ctx();
    if (ctx.state === "suspended") await ctx.resume();
    if (isG711AudioFrame(frame, mediaInfo)) {
        const decodedBuffer = buildG711AudioBuffer(ctx, frame, mediaInfo, { maxFrames: maxDecodeFrames });
        if (!decodedBuffer) {
            const err = new Error("Unable to decode G.711 audio data");
            err.diagnostics = {
                frameIndex: frame?.index ?? null,
                codecName: resolveAudioTrack(mediaInfo)?.codecName || frame?.codecName || null,
            };
            throw err;
        }
        const src = ctx.createBufferSource();
        src.buffer = decodedBuffer;
        src.connect(ctx.destination);
        src.start(0);
        return { ctx, source: src, durationSec: decodedBuffer.duration };
    }
    const attempts = [
        { maxFrames: maxDecodeFrames, aacAdtsMode: "auto" },
        { maxFrames: maxDecodeFrames, aacAdtsMode: "off" },
        { maxFrames: 1, aacAdtsMode: "auto" },
        { maxFrames: 1, aacAdtsMode: "off" },
        { maxFrames: Math.max(20, maxDecodeFrames), aacAdtsMode: "auto" },
    ];
    let decoded = null;
    let lastError = null;
    const diagnostics = [];
    for (const attempt of attempts) {
        const bytes = buildAudioPlaybackBytes(frame, mediaInfo, attempt);
        if (!(bytes instanceof Uint8Array) || bytes.length === 0) {
            diagnostics.push({ ...attempt, bytes: 0, ok: false, message: "empty-bytes" });
            continue;
        }
        try {
            decoded = await ctx.decodeAudioData(bytes.buffer.slice(0));
            diagnostics.push({ ...attempt, bytes: bytes.length, ok: true });
            break;
        } catch (err) {
            lastError = err;
            diagnostics.push({
                ...attempt,
                bytes: bytes.length,
                ok: false,
                message: err?.message || String(err),
            });
        }
    }
    if (!decoded) {
        const err = new Error(lastError?.message || "Unable to decode audio data");
        err.diagnostics = {
            frameIndex: frame?.index ?? null,
            codecName: resolveAudioTrack(mediaInfo)?.codecName || null,
            attempts: diagnostics,
        };
        throw err;
    }
    const src = ctx.createBufferSource();
    src.buffer = decoded;
    src.connect(ctx.destination);
    src.start(0);
    return { ctx, source: src, durationSec: decoded.duration };
}

export async function decodeAudioFramesToBufferWithWebAudio({
    frames,
    mediaInfo,
    audioContext = null,
    aacAdtsMode = "auto",
}) {
    const Ctx = globalThis.AudioContext || globalThis.webkitAudioContext;
    if (!Ctx) throw new Error("WebAudio API is not available in this browser.");
    const ctx = audioContext || new Ctx();
    if (ctx.state === "suspended") await ctx.resume();
    const inputFrames = Array.isArray(frames) ? frames.filter(Boolean) : [];
    if (!inputFrames.length) throw new Error("No audio frames to decode.");
    if (isG711AudioFrame(inputFrames[0], mediaInfo)) {
        const g711 = buildG711AudioBufferFromFrames(ctx, inputFrames, mediaInfo);
        if (!g711) throw new Error("Unable to decode G.711 audio data");
        return { ctx, buffer: g711, source: "g711" };
    }
    const bytes = buildAudioPlaybackBytesForFrameRange(inputFrames, mediaInfo, { aacAdtsMode });
    if (!(bytes instanceof Uint8Array) || bytes.length === 0) {
        throw new Error("No playable audio payload for selected segment.");
    }
    const decoded = await ctx.decodeAudioData(bytes.buffer.slice(0));
    return { ctx, buffer: decoded, source: "decodeAudioData" };
}

export const framePlaybackCodec = Object.freeze({
    pickPrimaryMediaResult,
    detectVideoCodecForPlayback,
    collectVideoFrames,
    collectAudioFrames,
    sliceFrameBytes,
    buildVideoDecodePlan,
    buildAudioPlaybackBytes,
    buildAudioPlaybackBytesForFrameRange,
    decodeVideoFramesToCanvas,
    playAudioFrameWithWebAudio,
    decodeAudioFramesToBufferWithWebAudio,
});
