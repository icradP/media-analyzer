...
import { annotateFieldRowsWithTiming } from "./timingFieldProvenance.js";
import {
    parseAvcLengthPrefixedNalUnits,
    parseAnnexBH264NalUnits,
} from "../codec/h264NaluScan.js";
import {
    parseHevcLengthPrefixedNalUnits,
    parseAnnexBHevcNalUnits,
} from "../codec/hevcNaluScan.js";
import {
...
function lookupRootsForFieldValues(fs) {
    const roots = [];
    const push = (x) => {
        if (x && typeof x === "object" && !roots.includes(x)) roots.push(x);
    };
    push(fs);
    push(fs?.sequenceHeader);
    push(fs?.es);
    push(fs?.pes);
    if (fs?.es?.nalus && typeof fs.es.nalus === "object") {
        for (const n of fs.es.nalus) push(n);
    }
    return roots;
}

function resolveFieldDisplayValue(fs, fieldPath) {
    if (!fs || !fieldPath) return null;
    const path = String(fieldPath);
    const leaf = path.includes(".") ? path.slice(path.lastIndexOf(".") + 1) : path;
    const parentPath = path.includes(".") ? path.slice(0, path.lastIndexOf(".")) : "";
    const valueKey = `_${leaf}_value`;
    const binaryKey = `${path}_binary`;
    const byteKey = `${path}_byte`;

    for (const root of lookupRootsForFieldValues(fs)) {
        const direct = getAtPath(root, path);
        if (direct !== undefined && direct !== null && typeof direct !== "object") {
            return String(direct);
        }
        const parent = parentPath ? getAtPath(root, parentPath) : root;
        if (parent && typeof parent === "object") {
            if (parent[leaf] !== undefined && parent[leaf] !== null && typeof parent[leaf] !== "object") {
                return String(parent[leaf]);
            }
            if (parent[valueKey] !== undefined && parent[valueKey] !== null) {
                const num = parent[valueKey];
                const label = parent[leaf];
                if (label !== undefined && label !== null && typeof label !== "object") return String(label);
                return String(num);
            }
            if (parent[binaryKey] !== undefined && parent[binaryKey] !== null) return String(parent[binaryKey]);
            if (parent[byteKey] !== undefined && parent[byteKey] !== null) return String(parent[byteKey]);
        }
        const topBinary = getAtPath(root, binaryKey);
        if (topBinary !== undefined && topBinary !== null && typeof topBinary !== "object") {
            return String(topBinary);
        }
    }
    return null;
}

function formatFieldHexSnippet(bytes, relOffset, length, maxBytes = 16) {
    if (!(bytes instanceof Uint8Array) || !Number.isFinite(relOffset) || !Number.isFinite(length) || length <= 0) {
        return null;
    }
    const start = Math.max(0, Math.floor(relOffset));
    const end = Math.min(bytes.length, start + Math.floor(length));
    if (end <= start) return null;
    const take = Math.min(end - start, maxBytes);
    const slice = bytes.subarray(start, start + take);
    const hex = Array.from(slice, (b) => b.toString(16).toUpperCase().padStart(2, "0")).join(" ");
    const suffix = end - start > take ? " …" : "";
    return `0x${hex}${suffix}`;
}

function enrichFieldRowsWithValues(rows, fs, fullBytes, absBase) {
    const base = Number.isFinite(absBase) ? absBase : 0;
    return (Array.isArray(rows) ? rows : []).map((row) => {
        const parsed = resolveFieldDisplayValue(fs, row.field);
        const rel = Number(row.offset) - base;
        const hex = formatFieldHexSnippet(fullBytes, rel, row.length);
        let value = parsed || hex || "—";
        if (parsed && hex && Number(row.length) <= 32) value = `${parsed} · ${hex}`;
        const bitMeta =
            Number.isFinite(row.bitLength) && row.bitLength > 0
                ? `bits +${row.bitOffset ?? 0}/${row.bitLength}`
                : null;
        return {
            ...row,
            parsedValue: parsed,
...
export function pickHexSourceBytesForInspector(rf, primaryBytes) {
    ...
    // Prefer container-level total length (e.g. FLV tag = 11 + dataSize + 4)
    // so the hex view covers ALL bytes that field offsets may reference.
    // Falls back to frame.size / dataSize when byteLength is unavailable.
    const len = Number.isFinite(fs?.byteLength)
        ? Number(fs.byteLength)
        : (Number.isFinite(rf?.size)
            ? Number(rf.size)
            : (Number.isFinite(fs?.dataSize) ? Number(fs.dataSize) : null));
...
function isFlvFrame(fs) {
    return !!(fs && (fs.tagType !== undefined || fs.tagTypeName !== undefined));
}

function isTsFrame(fs) {
    return !!(fs && (fs.pid !== undefined || fs.PES_packet_length !== undefined));
}

function buildTsPesFieldOffsets(rf, fs, fullBytes) {
    // Only synthesize PES field offsets for TS frames whose container parser
    // didn't already provide them — FLV / MP4 frames skip this entirely so the
    // tree doesn't fill up with unrelated `ts.pes.*` rows.
    if (!isTsFrame(fs)) return [];
    const rows = [];
...
export function buildFrameDetailForInspector(frame, result) {
    ...
    // Only augment with synthesized rows when the container parser didn't
    // already provide them (TS frames without exhaustive PES field offsets).
    // FLV / MP4 already expose rich, codec-aware fieldOffsets via their
    // parsers, so don't dilute the tree with redundant synthetic rows.
    if (isTsFrame(fs)) {
    ...
    return {
        ...
        flvTag,
        tsPes,
        fieldOffsets: annotateFieldRowsWithTiming(
            enrichFieldRowsWithValues(parsedFieldOffsets, fs, full, absBase),
        ),
        frameTimeProvenance: resolveFrameTimeProvenanceFromDetail(rf, fs),
    };
}

function resolveFrameTimeProvenanceFromDetail(rf, fs) {
    const usesComp = fs?.tagType === 9 || fs?.tagTypeName === "Video";
    if (typeof rf?.ptsTime === "number" && Number.isFinite(rf.ptsTime)) {
        return {
            primary: "ptsTime",
            formula: usesComp ? "ptsTime = (timestampFull + compositionTime) / 1000" : "ptsTime = timestampFull / 1000",
            fields: usesComp ? ["timestampFull", "compositionTime"] : ["timestampFull"],
        };
    }
    if (typeof rf?.dtsTime === "number" && Number.isFinite(rf.dtsTime)) {
        return { primary: "dtsTime", formula: "dtsTime = timestampFull / 1000", fields: ["timestampFull"] };
    }
    return { primary: null, formula: null, fields: [] };
}
