...
export class OnnxObjectDetector {
    ...
    async detectCanvas(canvas, options = {}) {
        ...
        const preprocessed = preprocessCanvasToNchw(canvas, {
            size,
            workspace: this.ensurePreprocessWorkspace(size),
        });
    ...
    async detectCanvasRois(canvas, rois, options = {}) {
        await this.init();
        const size = positiveInt(options.modelInputSize, this.modelInputSize);
        const scoreThreshold = finiteNumber(options.scoreThreshold, this.scoreThreshold);
        const iouThreshold = finiteNumber(options.iouThreshold, this.iouThreshold);
        const maxDetections = positiveInt(options.maxDetections, this.maxDetections);
        const maxRois = positiveInt(options.maxRois, 4);
        const roiList = normalizeRois(rois, canvas).slice(0, maxRois);
        const startedAt = performance.now();
        const detections = [];
        let runCount = 0;
        for (const roi of roiList) {
            if (detections.length >= maxDetections) break;
            const preprocessed = preprocessCanvasRoiToNchw(canvas, roi, {
                size,
                workspace: this.ensurePreprocessWorkspace(size),
            });
            const inputTensor = new this.ort.Tensor("float32", preprocessed.tensorData, [1, 3, size, size]);
            const rawOutputs = await this.session.run({ [this.inputName]: inputTensor });
            runCount += 1;
            const roiDetections = parseDetectionOutputs(rawOutputs, {
                labels: this.labels,
                meta: preprocessed.meta,
                scoreThreshold,
                iouThreshold,
            ...
            detections.push(...roiDetections);
        }
        const inferenceMs = performance.now() - startedAt;
        return {
            detections: nms(detections, iouThreshold).slice(0, maxDetections),
            provider: this.activeExecutionProvider,
            modelUrl: this.modelUrl,
            inputName: this.inputName,
            outputNames: this.outputNames,
    ...
    ensurePreprocessWorkspace(size) {
        this.preprocessWorkspace = createPreprocessWorkspace(size, this.preprocessWorkspace);
        return this.preprocessWorkspace;
    }
}
...
export class ObjectDetectionStabilizer {
    constructor(options = {}) {
        ...
        this.maxMissed = positiveInt(options.maxMissed, 60);
        this.resetFrameGap = positiveInt(options.resetFrameGap, 8);
        this.trackTtlMs = positiveInt(options.trackTtlMs, 2000);
        this.tracks = [];
        this.nextTrackId = 1;
    ...
    update(detections, options = {}) {
        ...
        const nowMs = finiteNumber(options.nowMs, typeof performance !== "undefined" ? performance.now() : Date.now());
        ...
        if (options.detectionRan === false) {
            this.pruneTracks(nowMs);
            return this.snapshotTracks(nowMs);
        }
        ...
        for (let i = 0; i < incoming.length; i++) {
            ...
            Object.assign(bestTrack, stable, {
                ...
                missCount: 0,
                lastSeen: nowMs,
                lastSeenFrame: Number.isFinite(frameIndex) ? frameIndex : bestTrack.lastSeenFrame,
        ...
        for (let i = 0; i < incoming.length; i++) {
            ...
            this.tracks.push({
                ...
                missCount: 0,
                lastSeen: nowMs,
                lastSeenFrame: Number.isFinite(frameIndex) ? frameIndex : null,
                hits: 1,
            });
            activeTracks.add(id);
        }
    ...
    pruneTracks(nowMs) {
        this.tracks = this.tracks.filter((track) => {
            const ageMs = nowMs - finiteNumber(track.lastSeen, nowMs);
            return ageMs <= this.trackTtlMs && (track.missed || 0) <= this.maxMissed && (Number(track.score) || 0) > 0.05;
        });
    }

    snapshotTracks(nowMs) {
        return this.tracks
            ...
            .map((track) => ({
                ...
                label: track.label,
                source: track.source,
                trackId: track.stableId || track.id,
                stableId: track.stableId || track.id,
                lastSeen: track.lastSeen ?? nowMs,
                lastSeenFrame: track.lastSeenFrame ?? null,
                missCount: track.missCount || track.missed || 0,
...
function createPreprocessWorkspace(size, previous = null) {
    const n = positiveInt(size, 640);
    const planeSize = n * n;
    if (
        previous?.size === n &&
        previous.workCanvas &&
        previous.ctx &&
        previous.tensorData instanceof Float32Array &&
        previous.tensorData.length === 3 * planeSize
    ) {
        return previous;
    }
...
export function preprocessCanvasToNchw(sourceCanvas, options = {}) {
    ...
    const workspace = createPreprocessWorkspace(size, options.workspace);
    const workCanvas = workspace.workCanvas;
    ...
    const tensorData = workspace.tensorData;
...
export function preprocessCanvasRoiToNchw(sourceCanvas, roi, options = {}) {
    if (!(sourceCanvas instanceof HTMLCanvasElement) && !(sourceCanvas instanceof OffscreenCanvas)) {
        throw new Error("A canvas with a decoded video frame is required.");
    }
    const sourceWidth = Math.max(1, Math.floor(sourceCanvas.width || 0));
    const sourceHeight = Math.max(1, Math.floor(sourceCanvas.height || 0));
    const r = clampRoi(roi, sourceWidth, sourceHeight);
    if (!r || r.width <= 1 || r.height <= 1) throw new Error("ROI is empty.");
    const size = positiveInt(options.size, 640);
    const workspace = createPreprocessWorkspace(size, options.workspace);
    const workCanvas = workspace.workCanvas;
    workCanvas.width = size;
    ...
    const drawWidth = Math.max(1, Math.round(r.width * scale));
    const drawHeight = Math.max(1, Math.round(r.height * scale));
    const padX = Math.floor((size - drawWidth) / 2);
    const padY = Math.floor((size - drawHeight) / 2);
    ctx.fillStyle = "rgb(0, 0, 0)";
    ctx.fillRect(0, 0, size, size);
    ctx.drawImage(sourceCanvas, r.x, r.y, r.width, r.height, padX, padY, drawWidth, drawHeight);
...
export function drawObjectDetections(canvas, detections, options = {}) {
    ...
    for (let i = 0; i < rows.length; i++) {
        ...
        const trackText = det.trackId || det.stableId ? ` #${det.trackId || det.stableId}` : "";
        const sourceText = options.sourceLabel || det.sourceLabel || det.source || "";
        const sourceSuffix = sourceText ? ` [${sourceText}]` : "";
        const label = `${det.label || "object"}${trackText}${sourceSuffix} ${(det.score * 100).toFixed(0)}%`;
...
function normalizeRois(rois, canvas) {
    const canvasWidth = positiveInt(canvas?.width, 0);
    const canvasHeight = positiveInt(canvas?.height, 0);
...
function roiPriority(roi) {
    return (Number(roi?.motionScore) || 0) * Math.max(1, Number(roi?.width) || 0) * Math.max(1, Number(roi?.height) || 0);
}
