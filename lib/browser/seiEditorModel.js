...
export function extractH264SeiFromFrame(frameWrapper, result, videoStream = null) {
    ...
    let nalus = [];
    let transport = "";
    const preferAvcc = preferredTransport === "avcc" || shouldPreferAvcc(rf, result);
    if (preferAvcc) {
...
export function insertH264SeiIntoFrame(fileBytes, frameWrapper, result, seiNaluBytes, videoStream = null) {
    ...
    if (formatName !== "flv") {
        throw new Error(`SEI insertion is currently supported for FLV/H.264 only. Current format=${formatName || "unknown"}.`);
    }
    const { payload, baseOffset, preferredTransport } = extractPayloadForNaluParse(rf, result);
...
function shouldPreferAvcc(rf, result) {
    const formatName = String(result?.format?.formatName || "").toLowerCase();
    const fs = rf?.formatSpecific || {};
...
function extractFirstH264SeiMessagePayload(seiNalu) {
    if (!(seiNalu instanceof Uint8Array) || seiNalu.length < 2 || (seiNalu[0] & 0x1f) !== 6) return null;
    ...
    while (off < rbsp.length - 1) {
        ...
        const payloadBytes = rbsp.slice(off, off + size.value);
        const detail = describeH264SeiPayload(type.value, payloadBytes);
        messages.push({
...
function concatBytes(parts) {
    ...
    for (const p of parts) {
        if (!(p instanceof Uint8Array) || !p.length) continue;
        out.set(p, off);
        off += p.length;
    }
    return out;
