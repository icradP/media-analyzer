...
function buildDecodeAttempts(plan, codecFamily) {
    ...
    const attempts = [{ name: "original", encodedFrames: base, description: plan?.description || null }];
    attempts.push({ name: "original-no-description", encodedFrames: cloneEncodedFrames(base), description: null });
    ...
    attempts.push({ name: "drop-aud", encodedFrames: withoutAud, description: plan?.description || null });
    attempts.push({ name: "drop-aud-no-description", encodedFrames: cloneEncodedFrames(withoutAud), description: null });
    ...
    if (firstNalus.length) {
        ...
        if (idrOnly) attempts.push({ name: "idr-only-no-description", encodedFrames: [{ type: "key", data: idrOnly }], description: null });
        if (inband) {
            attempts.push({ name: "inband-sps-pps-idr-no-description", encodedFrames: [{ type: "key", data: inband }], description: null });
            attempts.push({ name: "inband-sps-pps-idr-with-description", encodedFrames: [{ type: "key", data: inband }], description: plan?.description || null });
        }
    }
    return attempts;
}
...
export async function decodeVideoFrameWithStrategies({
    ...
    const attempts = buildDecodeAttempts(plan, plan?.codecFamily || null);
    const codecTries = codecRetryCandidates(codecString);
...
async function decodeGopByTinyH264Fallback({
    ...
    const renderStateId = tinyState.renderStateId;
