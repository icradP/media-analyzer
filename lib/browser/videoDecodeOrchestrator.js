import { normalizeVideoCodecLabel } from "../codec/videoCodecLabels.js";
...
export function codecCandidatesForStream(vStream) {
    const name = String(normalizeVideoCodecLabel(vStream?.codecName) || vStream?.codecName || "").toLowerCase();
    const cands = [];
    const push = (x) => {
...
export async function decodeVideoFrameWithStrategies({
    ...
    for (const codecTry of codecTries) {
        for (const at of attempts) {
            try {
                const decoded = await decodeVideoFramesToCanvas({
